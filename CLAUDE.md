...
In the Forge development environment, `api.asUser()` in resolvers causes a persistent `NEEDS_AUTHENTICATION_ERR` that cannot be recovered from without switching to `asApp()`. Prefer `api.asApp()` in resolvers and perform any required authorization checks manually using the relevant permission REST APIs.
...
The Confluence v1 Content Properties API (`/wiki/rest/api/content/{id}/property/...`) has been **removed** and returns a 410 GoneException. Always use the v2 API.

## Confluence Content Properties v2

Base path: `/wiki/api/v2/pages/{pageId}/properties`

Required scopes: `read:page:confluence` and `write:page:confluence`. The older `read:confluence-props` / `write:confluence-props` scopes do NOT cover the v2 endpoints.

**Reading a property by key requires two API calls** because:
- The list endpoint (`GET /wiki/api/v2/pages/{pageId}/properties`) returns `{ results: [{ id, key, version }] }` — the `value` field is absent.
- The single-property endpoint (`GET /wiki/api/v2/pages/{pageId}/properties/{propertyId}`) returns the full object including `value`, but uses a **numeric property ID** in the URL — NOT the key string.

Pattern to read a property by key:
1. GET the list, find the entry where `entry.key === yourKey`, extract `entry.id`.
2. GET the single property using the numeric `entry.id` to retrieve `value`.

**Writing properties:**
- Create: `POST /wiki/api/v2/pages/{pageId}/properties` — body `{ key, value }`
- Update: `PUT  /wiki/api/v2/pages/{pageId}/properties/{propertyId}` — body `{ key, value, version: { number: currentVersion + 1 } }`. The version increment is required, same as v1.

## route() is mandatory

All paths passed to `requestConfluence` (and other product fetch functions) in a backend resolver MUST be created with the `route` tagged template literal from `@forge/api`. Passing a plain string causes: "You must create your route using the 'route' export."

The `route` function also validates interpolated values and rejects anything containing path-separator characters (`/`, `\`, `..`). Always validate that page IDs are numeric strings before interpolating them (e.g. `if (!/^\d+$/.test(pageId)) throw ...`).

## Resolver module format

Use ESM imports for resolvers (`import Resolver from '@forge/resolver'`). CommonJS `require('@forge/resolver')` returns a module wrapper and causes "Resolver is not a constructor".

Handler paths in `manifest.yml` are resolved relative to `src/`. A handler declared as `resolvers/index.handler` maps to `src/resolvers/index.js`.

# Macro Config

The `ForgeReconciler.addConfig()` context (macro configuration panel) supports only a limited subset of UI Kit components.

**Not supported in macro config:**
- Layout components: `Stack`, `Inline`, `Box` — use a plain React fragment `<>` instead.
- Interactive components: `Button`, `LoadingButton`.

**Controlled inputs do not work in macro config.** The host injects saved config values into ALL named form fields and, due to a platform quirk, also injects into unnamed fields — serialising option objects as `[object Object]`. Rules:
- Never use a `value=` prop on a `Textfield` in macro config.
- Never use a `value=` / `onChange=` controlled pattern on a `Select` in macro config. The host fights the controlled value and the `onChange` handler may not fire reliably.
- Avoid extra `Textfield` components that have no `name` — the host may still inject a config value into them.

**Correct pattern for dependent dropdowns:**
Use `useConfig()`, which returns the **live** form state (updates in real-time as the user changes fields, before saving). Watch the upstream field value in a `useEffect` to load dependent options:

```js
const config = useConfig();
const pageId = typeof config?.pageId === 'object' ? config.pageId?.value : config?.pageId;

useEffect(() => {
  if (!pageId) return;
  invoke('getDependentOptions', { pageId }).then(setOptions);
}, [pageId]);
```

**Select saves option objects.** A `Select name="foo"` saves the full option object `{ label, value }` to config, not just the value string. When reading the saved value in view components, always normalise:
```js
const fieldValue = typeof config?.foo === 'object' ? config.foo?.value : config?.foo;
```
